...
fn test_start_game() {
    ...
    document.body().unwrap().append_child(&start_button).unwrap();

    let game_container = document.create_element("div").unwrap();
    game_container.set_attribute("id", "game").unwrap();
    document.body().unwrap().append_child(&game_container).unwrap();

    let answer_box_container = document.create_element("div").unwrap();
    answer_box_container.set_attribute("id", "answerBoxContainer").unwrap();
    document.body().unwrap().append_child(&answer_box_container).unwrap();

    let timer_display = document.create_element("div").unwrap();
    timer_display.set_attribute("id", "timerDisplay").unwrap();
    document.body().unwrap().append_child(&timer_display).unwrap();

    game_utils::start_game(&document, &secret_value, &timer_id);

    assert_eq!(start_button.get_attribute("style").unwrap(), "display: none;");
    assert_eq!(game_container.get_attribute("style").unwrap(), "display: block;");
    assert_eq!(answer_box_container.get_attribute("style").unwrap(), "display: block;");
}