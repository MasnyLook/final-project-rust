...
pub fn setup_start_button(
    ...
    let start_button = document
        ...
        .expect("startButton element not found");
    start_button
...
pub fn setup_click_button(
    ...
    let closure = Closure::wrap(Box::new(move || {
        let input_element = document_clone
            .get_element_by_id("inputNumber")
            .expect("inputNumber element not found");
        let input_value = input_element
            .dyn_ref::<HtmlInputElement>()
            .expect("inputNumber is not an HtmlInputElement")
            .value();
        let input_number: u32 = input_value.parse().unwrap_or(0);

        if !(1..=100).contains(&input_number) {
            window_clone
                .alert_with_message("Please enter a number between 1 and 100")
        ...
        let result_element = document_clone
            .get_element_by_id("result")
            .expect("result element not found");
    ...
    let button = document
        .get_element_by_id("click")
        .expect("click button not found");
    button
...
pub fn setup_restart_button(
    ...
    let start_button = document
        .get_element_by_id("restartButton")
        .expect("startButton element not found");
    start_button
        .add_event_listener_with_callback("click", start_closure.as_ref().unchecked_ref())
        .expect("Failed to add event listener");
...
pub fn setup_reroll_button(document: &Document, secret_value: &Rc<RefCell<u32>>) {
    ...
    let reroll_closure = Closure::wrap(Box::new(move || {
        *secret_value_clone.borrow_mut() = game_utils::generate_random_number() % 100 + 1;
        let result = document_clone
            .get_element_by_id("result")
            .expect("result element not found");
        result.set_text_content(None);
        let answerresult = document_clone
            .get_element_by_id("answerResult")
            .expect("result element not found");
        answerresult.set_text_content(Some("Rerolled!"));
        game_utils::update_attempts(&document_clone);
    }) as Box<dyn FnMut()>);

    let reroll_button = document
        .get_element_by_id("rerollButton")
        .expect("rerollButton element not found");
    reroll_button
        .add_event_listener_with_callback("click", reroll_closure.as_ref().unchecked_ref())
        .expect("Failed to add event listener");
