...
pub fn fetch_user_board(document: &Document, body: &Element) {
        ...
        match response {
            Ok(data) => {
                ...
                match results {
                    ...
                    Err(err) => {
                        window_clone
                            .alert_with_message("Error fetching user board")
                            .unwrap();
...
pub fn fetch_leaderboard(document: &Document, body: &Element) {
    let window = web_sys::window().unwrap();
    let document_clone = document.clone();
    let window_clone = window.clone();
    let body_clone = body.clone();
        ...
        match response {
            Ok(data) => {
                ...
                match results {
                    ...
                    Err(err) => {
                        window_clone
                            .alert_with_message("Error fetching leaderboard")
                            .unwrap();
                        web_sys::console::log_1(&format!("Error parsing JSON: {:?}", err).into());
                    }
                }
...
fn prepare_request_body(window: &web_sys::Window) -> String {
...
async fn send_getmessage_to_server(_window: &web_sys::Window) -> Result<String, reqwest::Error> {
    let client = Client::new();
...
pub fn create_table_from_results(
    document: &Document,
    body: &Element,
    results: Vec<models::GameResult>,
    is_username: bool,
) {
    ...
    for result in results {
        ...
        let timestamp_cell = html!(
            document,
            "td",
            {},
            &format_timestamp(
                result.timestamp.secs_since_epoch,
                result.timestamp.nanos_since_epoch
            )
        );
...
fn format_timestamp(secs_since_epoch: i64, nanos_since_epoch: i32) -> String {
    let naive = NaiveDateTime::from_timestamp_opt(secs_since_epoch, nanos_since_epoch as u32)
